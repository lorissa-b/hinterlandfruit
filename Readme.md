# Hinterland Fruits

Hinterland Fruits is the first in the Hinterland series of mods that aims to add a variety of new plants to the world. All are edible, though some will need to be processed in some way to avoid poisoning you. The eventual goal is full Biomes and Expanded Foods compatibility.

Also includes quality of life changes such as:
- Pruning shrubs with a knife as well as shears

## Crops
- Strawberry (Vanilla in 1.22) ❌
- Honeydew Melon
- Watermelon
- Cantaloupe
- Grape

## Trees
- Almond
- Apricot
- Avocado
- Banana
- Blood Orange
- Cashew
- Chestnut
- Plum
- Grapefruit
- Jackfruit
- Keylime
- Kumquat
- Lemon
- Lime
- Macadamia Nut
- Pawpaw
- Pecan
- Pistachio
- Persimmon
- Quince
- Walnut

## Bushes and Shrubs
- Guava (orange model)
- Saskatoon Berry 〰️ (blueberry model)
- Bearberry 〰️ (cranberry model)
- Chokeberry (blueberry model)
- Bunchberry
- Sea Buckthorn (cranberry model)
- Bay Leaf
- Honeyberry (model done)
- Golden Currant ✅
- Gooseberry (Vanilla in 1.22) ❌
- Jostaberry (blueberry model)
- Raspberry (Vanilla in 1.22) ❌
- Blackberry (Vanilla in 1.22) ❌
- Loganberry
- Elderberry
    - Elderflower
- Lingonberry (cranberry model)
- Huckleberry (blueberry model)
- Kiwi
- Dragonfruit
- Passionfruit
- Prickly Pear
- Muscadine